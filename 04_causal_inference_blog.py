...
states['carbon_intensity'] = states['State annual CO2 emissions (tons)'] / states['State annual net generation (MWh)']
...
model = smf.ols('carbon_intensity ~ treated + post + treat_post', data=states).fit(cov_type='cluster', cov_kwds={'groups': states['state_abbr']})
...
if did_pval < 0.05:
    logger.info(f"  by {abs(did_effect):.6f} tons/MWh ({abs(did_effect) / states['carbon_intensity'].mean() * 100:.1f}%)")
else:
    pass
states['years_to_treatment'] = states['data_year'] - treatment_year
for year in range(-5, 6):
    if year != -1:
        states[f'treated_year_{year}'] = (states['treated'] * (states['years_to_treatment'] == year)).astype(int)
formula = 'carbon_intensity ~ treated + ' + ' + '.join([f'treated_year_{y}' for y in range(-5, 6) if y != -1])
...
psm_data[psm_data['treated'] == 0]['propensity_score'].hist(bins=50, alpha=0.5, label='Control', color='blue')
psm_data[psm_data['treated'] == 1]['propensity_score'].hist(bins=50, alpha=0.5, label='Treated', color='red')
